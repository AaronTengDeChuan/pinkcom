...
local model_name = "flowqa_douban_use-bert-emb-%s_last-score-true_dropout-%s_valid%s_LS%s-%s_%s_minus1example_gc-0"
    % [use_bert_embeddings, std.toString(dropout)[:3], num_validation, num_lr_step, std.toString(lr_gamma)[:3], std.split(loss_fn, '.')[2]];
