...
class SMNPlainModel(nn.Module):
    """SMN Module contains ."""

    def __init__(self, config):
        super(SMNPlainModel, self).__init__()
        # hyperparameters
    ...
    def forward(self, inputs):
        ...
        # unstack
        all_utt_embeds = torch.unbind(all_utt_embeds, dim=1)
        # varname(all_utt_embeds)
        all_utt_lens = torch.unbind(all_utt_lens, dim=1)
        # varname(all_utt_lens)
        ...
        response_embeds = torch.transpose(response_embeds, 1, 2)
        # varname(response_embeds)
        resp_output = torch.transpose(resp_output, 1, 2)
        # varname(resp_output)
        matching_vectors = []
        for utt_embeds, utt_lens in zip(all_utt_embeds, all_utt_lens):
            ## calculate matrix1
            ## [None, 50, 200] * [None, 200, 50] -> [None, 50, 50]
            matrix1 = torch.matmul(utt_embeds, response_embeds)
        ...
        # Second Layer --- Matching Accumulation
            ...
            matching_vectors.append(dense_output)
        # varname(matching_vectors)
        ## push dense_output into final gru
        ## TODO: Why not mask the empty utterances ?
        final_output, last_hidden = self.final_gru(torch.stack(matching_vectors, dim=0))
...
if __name__ == "__main__":
    smnp = SMNPlainModel({"device": torch.device("cpu")})
    ...
    optimizer = AdamOptimizer({"lr": 0.001}).ops(smnp.parameters())
    for i in range(100):
        smnp.train()
        optimizer.zero_grad()
        logits = smnp(inputs)
